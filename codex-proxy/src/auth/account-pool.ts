...
export class AccountPool {
   ...
   * Deduplicates by credential identity, not shared ChatGPT account ID.
   * This allows multiple auth contexts for the same team account to coexist.
   */
  addAccount(token: string, refreshToken?: string | null): string {
    const accountId = extractChatGptAccountId(token);
    const profile = extractUserProfile(token);

    // Keep multiple auth contexts for one shared account, but avoid duplicating
    // the exact same credential on repeated imports.
    for (const existing of this.accounts.values()) {
      const sameRefreshToken = !!refreshToken && existing.refreshToken === refreshToken;
      const sameAccessToken = existing.token === token;
      if (sameRefreshToken || sameAccessToken) {
