<template>
  ...
  >
    ...
    <div class="sidebar-header">
      ...
      <div class="flex h-9 w-9 items-center justify-center overflow-hidden rounded-lg border border-slate-200 bg-white dark:border-white/10 dark:bg-white/5">
    ...
    <!-- Bottom Section -->
    <div class="mt-auto border-t border-slate-200 p-3 dark:border-white/10">
      <!-- Collapse Button -->
      <button
        @click="toggleSidebar"
    ...
    h(
      ...
      [
        h('path', {
          'stroke-linecap': 'round',
          'stroke-linejoin': 'round',
          d: 'M18 18.72a8.96 8.96 0 01-12 0m12 0a8.96 8.96 0 00-12 0m12 0V21m-12-2.28V21m12-2.28A6.75 6.75 0 0012 15.75a6.75 6.75 0 00-6 2.97M15.75 6.75a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM19.5 8.25h1.5m-1.5 0a2.25 2.25 0 11-4.5 0m4.5 0a2.25 2.25 0 00-4.5 0M4.5 8.25H3m1.5 0a2.25 2.25 0 104.5 0m-4.5 0a2.25 2.25 0 014.5 0'
        })
      ]
    )
}
...
const customMenuItemsForUser = computed(() => {
  ...
  const purchaseUrl = appStore.cachedPublicSettings?.purchase_subscription_url?.trim()
  const purchaseLabel = t('nav.buySubscription')

  return items
    .filter((item) => item.visibility === 'user')
    .filter((item) => {
      if (!appStore.cachedPublicSettings?.purchase_subscription_enabled) return true
      const sameLabel = item.label.trim() === purchaseLabel
      const sameUrl = Boolean(purchaseUrl && item.url.trim() === purchaseUrl)
      return !sameLabel && !sameUrl
    })
