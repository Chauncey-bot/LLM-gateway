...
onMounted(async () => {
  ...
  if (registerDataStr) {
    try {
      ...
      referralCode.value = registerData.referral_code || sessionStorage.getItem('pending_referral_code') || ''
...
async function bindPendingReferral(): Promise<void> {
  const code = referralCode.value || sessionStorage.getItem('pending_referral_code')
  if (!code) return

  try {
    await bindReferralRegistration(code)
