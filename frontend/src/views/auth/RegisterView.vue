<template>
  <AuthLayout variant="ad" card-class="max-h-[calc(100vh-5rem)] overflow-y-auto">
    <div class="space-y-6">
      <!-- Title -->
...
import { bindReferralRegistration } from '@/api/referrals'
...
const siteName = ref<string>('PYTHAGORAS')
const linuxdoOAuthEnabled = ref<boolean>(false)
const registrationEmailSuffixWhitelist = ref<string[]>([])
const pendingReferralCode = ref<string>('')
...
onMounted(async () => {
  try {
    const referralParam = Array.isArray(route.query.ref) ? route.query.ref[0] : route.query.ref
    if (referralParam) {
      pendingReferralCode.value = referralParam.trim()
      sessionStorage.setItem('pending_referral_code', pendingReferralCode.value)
    } else {
      pendingReferralCode.value = sessionStorage.getItem('pending_referral_code') || ''
    }
    formData.referral_code = pendingReferralCode.value
    ...
    siteName.value = settings.site_name || 'PYTHAGORAS'
...
function handleReferralCodeInput(): void {
  pendingReferralCode.value = formData.referral_code.trim()
  if (pendingReferralCode.value) {
    sessionStorage.setItem('pending_referral_code', pendingReferralCode.value)
  } else {
...
async function bindPendingReferral(): Promise<void> {
  const referralCode = formData.referral_code || pendingReferralCode.value || sessionStorage.getItem('pending_referral_code')
  if (!referralCode) return

  try {
    await bindReferralRegistration(referralCode)
    sessionStorage.removeItem('pending_referral_code')
    pendingReferralCode.value = ''
  } catch (error) {
    console.warn('Failed to bind referral registration:', error)
  }
}
</script>
