<template>
  <AppLayout>
    <div class="console-page space-y-5">
      <div class="console-title-panel flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
          <p class="console-kicker">{{ t('nav.myAccount') }}</p>
          <h1 class="console-section-title">{{ t('nav.apiKeys') }}</h1>
          <p class="console-section-description">{{ t('keys.description') }}</p>
        </div>
        ...
        <div class="api-key-record-list">
          <template v-if="loading">
            <div
              v-for="i in 5"
              :key="i"
              class="api-key-record"
            >
              <div class="mb-4 flex items-center justify-between gap-4">
                <div class="h-5 w-44 animate-pulse rounded bg-gray-200 dark:bg-dark-700"></div>
                <div class="h-8 w-52 animate-pulse rounded bg-gray-200 dark:bg-dark-700"></div>
              </div>
          ...
          >
            <div class="api-key-record-main">
              <div class="min-w-0">
                ...
                <div class="flex min-w-0 items-center gap-2">
                  <code class="api-key-full-key">{{ row.key }}</code>
                  <button
                    @click="copyToClipboard(row.key, row.id)"
                    class="shrink-0 rounded-lg p-1 transition-colors hover:bg-gray-100 dark:hover:bg-dark-700"
                    :class="
                      copiedKeyId === row.id
                        ? 'text-green-500'
              ...
              <div class="api-key-record-actions">
                <button
                  @click="openUseKeyModal(row)"
                  class="api-key-action text-green-600 hover:bg-green-50 dark:text-green-400 dark:hover:bg-green-900/20"
                  :title="t('keys.useKey')"
                >
                ...
                <button
                  ...
                  class="api-key-action text-blue-600 hover:bg-blue-50 dark:text-blue-400 dark:hover:bg-blue-900/20"
                ...
                <button
                  @click="editKey(row)"
                  class="api-key-action text-gray-600 hover:bg-gray-100 hover:text-primary-600 dark:text-gray-300 dark:hover:bg-dark-700 dark:hover:text-primary-400"
                  :title="t('common.edit')"
                >
                ...
                <button
                  ...
                  class="api-key-action text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-900/20"
            ...
            <div class="api-key-record-details">
              <div class="api-key-detail api-key-detail-group">
                <span>{{ t('keys.group') }}</span>
                <button
                  :ref="(el) => setGroupButtonRef(row.id, el)"
                  @click="openGroupSelector(row)"
                  class="api-key-group-button"
              ...
              <div class="api-key-detail">
                ...
                <strong :class="row.expires_at && new Date(row.expires_at) < new Date() ? 'text-red-500 dark:text-red-400' : ''">
              ...
              <div class="api-key-detail">
                ...
                <strong>{{ row.last_used_at ? formatDateTime(row.last_used_at) : '-' }}</strong>
              </div>

              <div class="api-key-detail">
                <span>{{ t('keys.created') }}</span>
                <strong>{{ formatDateTime(row.created_at) }}</strong>
              </div>

              <div class="api-key-detail api-key-detail-rate-limit">
                <span>{{ t('keys.rateLimitColumn') }}</span>
                <div v-if="hasRateLimit(row)" class="space-y-1">
                  <div
                    v-for="item in rateLimitItems(row)"
                    :key="item.label"
                    class="grid grid-cols-[2rem_minmax(0,1fr)_auto] items-center gap-2 text-xs"
                  >
                    <span class="text-gray-500 dark:text-gray-400">{{ item.label }}</span>
                    <div class="h-1 overflow-hidden rounded-full bg-gray-200 dark:bg-dark-600">
                      <div :class="item.barClass" :style="{ width: item.percent + '%' }" />
                    </div>
                    <span :class="item.textClass">{{ item.value }}</span>
                  </div>
                  <button
                    v-if="row.usage_5h > 0 || row.usage_1d > 0 || row.usage_7d > 0"
                    @click.stop="confirmResetRateLimitFromTable(row)"
                    class="inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-xs text-gray-500 transition-colors hover:bg-gray-100 hover:text-primary-600 dark:hover:bg-dark-700 dark:hover:text-primary-400"
                ...
                <strong v-else class="text-gray-400 dark:text-dark-500">-</strong>
              </div>
            </div>
          </article>
        </div>
...
function quotaPercent(row: ApiKey): number {
  ...
  return Math.min(((row.quota_used || 0) / row.quota) * 100, 100)
}

function quotaTextClass(row: ApiKey): string[] {
  return [
    'font-medium',
...
function quotaBarClass(row: ApiKey): string[] {
  return [
    'h-full rounded-full transition-all',
    row.quota_used >= row.quota
      ? 'bg-red-500'
...
function hasRateLimit(row: ApiKey): boolean {
  return row.rate_limit_5h > 0 || row.rate_limit_1d > 0 || row.rate_limit_7d > 0
}

function rateLimitVisualClass(usage: number, limit: number): string {
  if (usage >= limit) return 'text-red-500'
  if (usage >= limit * 0.8) return 'text-yellow-500'
  return 'text-gray-700 dark:text-gray-300'
}

function rateLimitBarClass(usage: number, limit: number): string[] {
  return [
    'h-full rounded-full transition-all',
    usage >= limit ? 'bg-red-500' : usage >= limit * 0.8 ? 'bg-yellow-500' : 'bg-emerald-500',
  ]
}

function rateLimitItems(row: ApiKey) {
  return [
    {
      label: '5h',
    ...
    .map((item) => ({
      label: item.label,
      value: `$${item.usage.toFixed(2)}/$${item.limit.toFixed(2)}`,
      percent: Math.min((item.usage / item.limit) * 100, 100),
      textClass: ['font-medium tabular-nums', rateLimitVisualClass(item.usage, item.limit)],
      barClass: rateLimitBarClass(item.usage, item.limit),
      resetText: item.resetAt ? formatResetTime(item.resetAt) : '',
    }))
}
...
.api-key-record {
  @apply min-w-0 rounded-lg border border-gray-200 bg-white p-4 shadow-lg shadow-slate-900/10 transition-shadow hover:shadow-xl dark:border-dark-700 dark:bg-dark-900 dark:shadow-black/30;
}

.api-key-record-main {
  @apply flex flex-col gap-3 border-b border-gray-100 pb-3 dark:border-dark-700;
}

.api-key-record-actions {
  @apply flex flex-wrap items-center gap-1.5;
}

.api-key-action {
  @apply inline-flex h-8 items-center gap-1.5 rounded-lg px-2.5 text-xs font-medium transition-colors;
}

.api-key-full-key {
  @apply code min-w-0 flex-1 whitespace-normal break-all text-xs leading-5;
}

.api-key-record-details {
  @apply grid gap-3 pt-3 md:grid-cols-2;
}

.api-key-detail {
  @apply min-w-0 rounded-lg bg-gray-50/80 px-3 py-2 dark:bg-dark-800/70;
}
...
.api-key-detail > strong {
  @apply block break-words text-sm font-semibold leading-5 text-gray-900 dark:text-gray-100;
}

.api-key-detail-group {
  @apply flex items-center justify-between gap-2 bg-gray-50/80 px-3 py-2 dark:bg-dark-800/70;
}

.api-key-detail-group > span {
  @apply mb-0 shrink-0;
}

.api-key-group-button {
  @apply flex min-w-0 flex-1 items-center justify-end gap-2 rounded-lg px-2 py-1 transition-colors hover:bg-gray-100 dark:hover:bg-dark-700;
}
